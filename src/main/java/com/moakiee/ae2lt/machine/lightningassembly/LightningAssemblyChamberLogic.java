 ...
 */
public final class LightningAssemblyChamberLogic implements IGridTickable {
    ...
    public TickRateModulation tickingRequest(IGridNode node, int ticksSinceLastCall) {
        ...
        Optional<LightningAssemblyLockedRecipe> lockedRecipe = host.getLockedRecipe();
        if (lockedRecipe.isEmpty()) {
            host.setWorking(false);
            return TickRateModulation.SLEEP;
        }

        Optional<LightningAssemblyRecipeCandidate> lockedCandidate = validateLockedRecipe(lockedRecipe.get());
    ...
    public long computeEnergyToConsumeThisTick(LightningAssemblyLockedRecipe lockedRecipe) {
    ...
    private TickRateModulation tickActiveRecipe(
            ...
            LightningAssemblyRecipeCandidate lockedCandidate) {
    ...
    private void tryStartProcessing() {
        Optional<LightningAssemblyLockedRecipe> lockedRecipe = host.lockCurrentRecipe();
        if (lockedRecipe.isEmpty()) {
    ...
    private Optional<LightningAssemblyRecipeCandidate> validateLockedRecipe(LightningAssemblyLockedRecipe lockedRecipe) {
    ...
    private void completeRecipe(LightningAssemblyRecipeCandidate lockedCandidate) {
