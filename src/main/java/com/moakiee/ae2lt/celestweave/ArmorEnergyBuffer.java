...
public final class ArmorEnergyBuffer {
    ...
    public static long read(ItemStack stack, HolderLookup.Provider registries) {
        return Math.max(0L, Math.min(
                capacity(stack, registries), ModDataComponents.getCelestweaveEnergyBuffer(stack)));
    }
    ...
    public static void write(ItemStack stack, HolderLookup.Provider registries, long value) {
        ...
        ModDataComponents.setCelestweaveEnergyBuffer(
                stack, Math.max(0L, Math.min(capacity(stack, registries), value)));
    }
